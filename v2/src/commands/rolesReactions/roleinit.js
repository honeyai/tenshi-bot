  ...
  run: async (client, message, args) => {
    if (args.length !== 1) {
      ...
      try {
        ...
        let thisGuild = message.guild.id;
        let allChannels;
        let textChannels;
        try {
          allChannels = await client.guilds.fetch(thisGuild);
          textChannels = await allChannels.channels.cache.filter(
            (channel) => channel.type === "text"
          );
          
          let fetched
          textChannels.each((msg) => {
            msg.messages.cache.each(
              (thing) => {
                if (thing.id === args[0]) {
                  fetched = thing;
                }
          ...
          if (fetched) {
            ...
            collector.on("collect", (msg) => {
              ...
              let [emojiName, roleName] = msg.content
                .toLowerCase()
                .split(/,\s+/);
              ...
              if (!emojiName || !roleName)
                ...
                fetched
                  .react(emojiFind)
                  .then((emoji) =>
                    console.log("reacted with " + emojiFind + ".")
                  )
            ...
            collector.on("end", async (collected, reason) => {
              ...
              if (findDoc) {
                ...
                message.channel.send(
                  "You're trying to initialize a message that's already set up."
                );
              } else {
